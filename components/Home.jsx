...
import { delay, useAnimate, usePresence, stagger,useTransform, useInView  } from "framer-motion";
...
const Lobby = ({blogs,setslideshow,openmodal}) => {
  ...
  >
    ...
    <motion.div
      ...
      whileHover={{rotate:0,zIndex:20,transition:{type:"tween",stiffness:200,damping:600}}}
    ...
    >
            <motion.button onClick={()=>openmodal()}  whileHover={{scale:1.1}} whileTap={{scale:.9}} className=' max-[550px]:text-sm w-max px-2 py-1 bg-rose-700   rounded-md mt-2 border border-rose-100 text-slate-100 '>Create lobby</motion.button>
      <motion.img  animate={{y:[0,10,0,10,0]}} transition={{ease:"easeInOut",repeat:Infinity,duration:3}} className='w-[50px] h-[50px] sm:w-[100px] sm:h-[100px] mr-24 ' src="./winyaka.png" alt="" />
    ...
    >
      <motion.img  animate={{y:[0,10,0,10,0]}} transition={{ease:"easeInOut",repeat:Infinity,duration:3}} className=' w-[50px] h-[50px] sm:w-[100px] sm:h-[100px] ' src="./EulaCard.png" alt="" />
      <motion.button onClick={()=>setslideshow()}  whileHover={{scale:1.1}} whileTap={{scale:.9}} className=' max-[550px]:text-sm w-max px-2 py-1 bg-cyan-200   rounded-md mt-2 border border-cyan-500 text-slate-800 '>Join lobby</motion.button>
    </motion.div>
    </div>
    ...
    {/* <motion.button
      ...
      whileHover={() =>
        animate(scope.current, { rotate: 45 }, { duration: 1 })
      }
      className=" bg-red-600 w-max"
    >
      click
