...
const Room = ({searchParams}) => {
   ...
   const [crews,setcrews]=useState([{socketid:null,name:null,character:"Eula"},{socketid:null,name:null,character:"Default"},{socketid:null,name:null,character:"Default"},{socketid:null,name:null,character:"Default"}])
   ...
   useEffect(()=>{
        ...
        Socket&&Socket.emit("send", {  room:"6789"}, (error) => {
          if (error) {
            alert(error);
          }
        })
        Socket&&Socket.on("bosses",(data)=>{
          console.log(data)
          setboss(data.boss)
  ...
  return (
    ...
    <div className='marcellus w-screen h-screen  relative flex justify-between charsMain ' >
        ...
        {Characters.map((c,i)=>(
            <div key={i} className=" w-[80px] flex flex-col justify-center items-center m-2 rounded-md overflow-hidden border border-white">
  <div style={{backgroundColor:`${colors[c.vision]}`}}  className={` rounded-br-xl w-full h-[80px]`}>
<img className=' w-full h-full border border-orange-800  ' src={`../characters/${c.character}.png`} alt="" />

            </div>
            <div className=" bg-slate-800 w-full h-4 flex justify-center"><p className=' text-xs text-slate-200 font-semibold' >{c.character}</p></div>
            </div>
        ...
        <div className="slots absolute  flex bottom-0  w-[60%] h-[25%] justify-center items-center border-2 border-orange-400 rounded-md">
          ...
          {crews&&crews.map((c,i)=> <div key={i} className="slotcard  w-[100px] h-[130px] border border-rose-400 rounded-md mx-2 my-2 overflow-hidden ">
                <img className=' charImg w-full h-[100px]  border-b border-white ' src={c.character!=="Default"?`../characters/${c.character}.png`:"../paimon.png"} alt="" />
                <div className=" h-[30px] flex justify-center">
                {c.character!=="Default"&&<p className=' w-max h-4 text-white'>{c.character}</p>}
                </div>
            ...
            <div className=" dice flex justify-center items-center ml-12">
                <button onClick={()=>switchchar()} className='  text-slate-200 w-[50px] h-[50px]'>
        ...
        <div className="boss-main w-[30%] flex justify-between items-center h-full">
            ...
            <div className=" w-[100%] flex flex-col justify-center items-center">
              <div className="boosbg w-[200px] h-[230px] rounded-md  flex flex-col  items-center justify-between border border-white overflow-hidden">
                ...
                <img className=' w-full h-full' src={boss?`../bosses/${boss}.jpg`:"../bosslogo.png"} alt="" />
                </div>
                </div>
                <div className=" w-full h-[20px] flex justify-center  overflow-hidden"> <p className=' text-white '>{boss}</p></div>
             
             
            <button onClick={()=>switchboss()} className='text-slate-200 w-[50px] h-[50px] mt-4'>       <img src="../dice.png" alt="" /></button>
            </div>
        </div>
            ...
            {Characters.map((c,i)=>(
                <div key={i} className=" w-[80px] flex flex-col justify-center items-center m-2 rounded-md overflow-hidden border border-white">
      <div style={{backgroundColor:`${colors[c.vision]}`}}  className={` rounded-br-xl w-full h-[80px]`}>
    <img className=' w-full h-full  ' src={`../characters/${c.character}.png`} alt="" />
