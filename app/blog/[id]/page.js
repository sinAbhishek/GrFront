...
function Page({ searchParams }) {
  ...
  return (
    <div className="w-screen flex justify-center bg-slate-900">
      {filter[0] && (
        <div className=" w-8/12 h-max flex flex-col mt-8 text-white">
          <div className="w-full flex justify-center">
            <h1 className="kanit   text-3xl text-slate-100">
              {filter[0].title}
            </h1>
          </div>
          ...
          <div className=" flex justify-between items-center px-2">
            <div className="">
              <h6 className=" text-slate-100 text-md font-semibold ">
                {filter[0].name}
              </h6>
              {time && (
                <h6 className="italic text-sm text-slate-100">
                  {time.toDateString()}
                </h6>
              )}
            </div>
          ...
          <p className="crimson mt-12 text-lg">{filter[0].description}</p>
        </div>
      )}
    </div>
  );
